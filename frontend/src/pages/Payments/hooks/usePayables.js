  ...
  const handlePayableSubmit = useCallback(async (values, editingPayable) => {
    try {
      ...
      if (editingPayable) {
        ...
        // 创建新应付管理
        const createResponse = await apiClient.post('/payment', values);
        if (createResponse && (createResponse.success || createResponse.data?.success)) {
          message.success('创建成功');
          // 创建成功后刷新列表
          fetchPayables();
        } else {
          const errMsg = createResponse?.message || createResponse?.data?.message || '创建失败';
