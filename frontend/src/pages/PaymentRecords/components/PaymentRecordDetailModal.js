...
const PaymentRecordDetailModal = ({
  ...
  const refreshAttachments = async () => {
    const id = viewingPaymentRecord?.Id || viewingPaymentRecord?.id;
    if (!id) return;
    try {
      setLoading(true);
      const response = await apiClient.get(`/attachment/payment/${id}`);
      if (response.success) {
  ...
  return (
    <Modal
      title={<div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>付款记录详情</div>}
      open={visible}
      onCancel={onCancel}
      footer={[
              ...
              <Row gutter={[8, 12]}>
                <Col span={12}>
                  <div style={{ marginBottom: '8px' }}>
                    ...
                    <span style={{ color: '#1890ff', fontWeight: 'bold', fontFamily: 'monospace' }}>
              ...
              <Row gutter={[8, 12]}>
                <Col span={12}>
                  <div style={{ marginBottom: '8px' }}>
                    ...
                    <div style={{ marginTop: '4px', fontSize: '18px', fontWeight: 'bold', color: '#52c41a' }}>
                      {(viewingPaymentRecord.CurrencySymbol || '')}
                      {(viewingPaymentRecord.PaymentAmount || viewingPaymentRecord.paymentAmount || 0).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                  </div>
                </Col>
                <Col span={12}>
                  <div style={{ marginBottom: '8px' }}>
                    ...
                    <div style={{ marginTop: '4px', fontSize: '16px', fontWeight: 'bold', color: '#1890ff' }}>
                      ${convertToUSD(viewingPaymentRecord.PaymentAmount || viewingPaymentRecord.paymentAmount, viewingPaymentRecord.CurrencyCode || viewingPaymentRecord.currencyCode).toFixed(2)}
                    </div>
                  </div>
                </Col>
                <Col span={24}>
        ...
        <Row gutter={24}>
          <Col span={24}>
            <Card size="small" title="备注信息" style={{ marginBottom: '16px' }}>
              <div style={{ padding: '12px', backgroundColor: '#fafafa', borderRadius: '4px', border: '1px solid #d9d9d9', minHeight: '60px' }}>
                {viewingPaymentRecord.Notes || viewingPaymentRecord.notes ? (
                  <span style={{ color: '#333', lineHeight: '1.6' }}>
                    {viewingPaymentRecord.Notes || viewingPaymentRecord.notes}
                  </span>
                ) : (
                  <span style={{ color: '#999', fontStyle: 'italic' }}>暂无备注信息</span>
                )}
              </div>
            </Card>
          </Col>
