...
router.post('/', authenticateToken, [
  ...
  try {
    ...
    if (parseFloat(finalPaymentAmount) > remainingAmount) {
      return res.status(400).json({
        success: false,
    ...
    const result = await transaction(async (connection) => {
      ...
      // 3. 更新应付管理状态
      const newTotalPaid = parseFloat(totalPaid[0].totalPaid) + parseFloat(finalPaymentAmount);
      ...
      await connection.execute(`
        ...
        WHERE Id = ?
      `, [newStatus, finalPayableManagementId]);
